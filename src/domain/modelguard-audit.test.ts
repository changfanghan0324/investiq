...
describe("ModelGuard deterministic audit", () => {
  it("keeps the clean fixture free of critical issues", async () => {
    ...
    expect(report.issues.every((issue) => issue.ruleId !== "FORMULA_MISSING_SHEET")).toBe(true);
  });
  ...
  it("surfaces broken references in the error fixture", async () => {
    ...
    expect(report.issues.some((issue) => issue.ruleId === "FORMULA_MISSING_CELL")).toBe(true);
    expect(report.issues.some((issue) => issue.ruleId === "HIDDEN_SHEET_REVIEW")).toBe(true);
  });
  ...
  it("guards CSV formula injection", () => {
    const report = { schemaVersion: "1.0" as const, generatedAt: "2026-08-10T00:00:00.000Z", fileName: "=unsafe.xlsx", workbookSha256: "0", provenance: { sourceType: "user-local" as const, provider: "browser" as const, generatedAt: "2026-08-10T00:00:00.000Z", disclaimer: "local" }, issues: [{ id: "1", ruleId: "X", severity: "warning" as const, category: "formula" as const, title: "=formula", message: "safe", observed: "=unsafe" }], summary: { critical: 0, warning: 1, info: 0 } };
